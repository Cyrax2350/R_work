...
WHERE p.project_id IN(617823,610400,610100,600200,617828,604400,614000,607600,601500,617812,606400,613700,618289,613900,618223,602800,618237,601700,601300,
  603100,604700,600900,617804,617805,610300,618164,602400,613300,609000,617923,609900,617802,617845,617789,601000,612100,609600,615000,601400,602500,604100,
    601900,602200,601200,603000,604900,617850,605300,617929,602000)
...
if(file.exists('output/CIBO_projects.geojson')) {file.remove('output/CIBO_projects.geojson')}
st_write(sfc, "output/CIBO_projects.geojson")

if(file.exists('output/CIBO_projects.kml')) {file.remove('output/CIBO_projects.kml')}
st_write(sfc, "output/CIBO_projects.kml")

if(file.exists('output/CIBO_projects.shp')) {file.remove('output/CIBO_projects.shp')}
st_write(sfc, "output/CIBO_projects.shp")
